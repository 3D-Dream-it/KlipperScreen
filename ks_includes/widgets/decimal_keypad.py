...
class DecimalKeypad(Gtk.Box):
    def __init__(self, screen, change_temp, close_function, title=None):
        ...
        if title:
            self.labels['title'] = Gtk.Label(title)
            self.labels['title'].set_halign(Gtk.Align.CENTER)
            self.labels['title'].set_vexpand(False)
        ...
        c = self._gtk.Button('complete', _('Accept'), None, .66, Gtk.PositionType.LEFT, 1)
        c.connect("clicked", self.accept_value)

        if title:
            self.add(self.labels['title'])
